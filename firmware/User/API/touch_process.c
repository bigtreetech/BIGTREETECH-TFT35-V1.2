...
void TSC_Calibration(void)
{
	...
	}while(calibrationEnsure(240,160)==0);
...
u16 KEY_GetValue(u8 total_rect,const GUI_RECT* menuRect)
{
    ...
    static u16   first_key =IDLE_TOUCH;
...
//			if(space_time==10)
//            if(first_key == key_num)
                key_return = key_num;
            key_num = IDLE_TOUCH;
            first_key = IDLE_TOUCH;
            firstPress = true;
        }
    }
    return key_return;
...
#define KEY_DOUOBLE_SPACE        15     //多长时间内点击两下判定为双击
#define KEY_LONG_PRESS_START     200     //连续按多久开始判定为 长按 键值

#define KEY_LONG_PRESS_SPACE_MAX 10     //长按时 最长多久返回一次键值
#define KEY_LONG_PRESS_SPACE_MIN 2      //长按时 最短多久返回一次键值
...
//    
//    static KEY_STATUS nowStatus = NO_CLICK;    //按键当前的状态
