...
bool powerOffGetData(void)
{
	...
	{        
		mustStoreCmd("G1 Z%d\n", limitValue(0,infoBreakPoint.axis[Z]+10,400));
        mustStoreCmd("M83\n");
		mustStoreCmd("G1 E30 F300\n");
		mustStoreCmd("G1 E-10 F4800\n");
        mustStoreCmd("G1 X%.5f Y%.5f Z%.5f F3000\n",infoBreakPoint.axis[X],infoBreakPoint.axis[Y],infoBreakPoint.axis[Z]);                 
		mustStoreCmd("G1 E10 F4800\n");
		mustStoreCmd("G92 E%.5f\n",infoBreakPoint.axis[E]);
        ...
        {
            mustStoreCmd("G90\n"); 
            if(infoBreakPoint.relative_e == true)
            {
                mustStoreCmd("M83\n");
            }
        }   
