...
u8 SD_SPI_Read_Write_Byte( u8 data)	
{
	...
	SPI2->DR = (u16)data; //通过外设SPIx发送一个数据
	retry=0;
...
void SD_SPI_Init(void)
{
  ...
  /*PD2是SD卡片选信号*/
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB,ENABLE);//使能PORTD时钟
	
	GPIO_InitStructure.GPIO_Mode=GPIO_Mode_Out_PP;		//输出模式
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_12;				
	GPIO_InitStructure.GPIO_Speed=GPIO_Speed_50MHz;
	GPIO_Init(GPIOB,&GPIO_InitStructure);					//将PD2设置为上拉推挽输出，用来片选SD卡
//	W25QXX_Init();
	SPI2_Init();					 //SPI2初始化
	SD_CS_H();					//取消片选
	//SPI2_SetSpeed()
...
{	
	...
	if(cmd!=0XFD)//不是结束指令
	{
		for(t=0;t<512;t++)SD_SPI_Read_Write_Byte(buf[t]);//提高速度,减少函数传参时间
