...
const GUI_RECT iconUpdateRect = {192, 112, 192 + ICON_WIDTH, 112 + ICON_HEIGHT};
const GUI_RECT labelUpdateRect = {0, 112 + ICON_HEIGHT, LCD_WIDTH, 112 + ICON_HEIGHT + BYTE_HEIGHT};
...
"Percentage",  "BabyStep","Mmm_001","OnBoardSD","OnTFTSD"
}; 
...
void updateFont(char *font, u32 addr)
{
  ...
  GUI_DispString(40, 140, (u8*)"Updating:   %",0);
	...
	while(!f_eof(&myfp))
	{
		res=f_read(&myfp,tempbuf,4096,&rnum);
		if(res!=FR_OK) break;
...
void scanUpdates(void)
{
  if(mountSDCard())
  {
    u8 result = scanUpdateFile();		//ɨ���Ŀ¼�µ��ļ�
    if (result & BMP)			//bmp�ļ���
    {
      updateIcon();					
    }
